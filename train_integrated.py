...
def train_integrated():
    ...
    # 모델 이름 설정
    model_name = "AfterimageVAE_IntegratedTraining"
    ...
    logger.info("Starting AfterimageVAE_IntegratedTraining training...")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.val_freq == 0:
            ...
            # 학습률 조정 (최종 출력의 MSE 기준)
            scheduler.step(val_metrics['final']['MSE'])
            
            # 최고 모델 저장
            if val_metrics['final']['MSE'] < best_val_mse:
                best_val_mse = val_metrics['final']['MSE']
