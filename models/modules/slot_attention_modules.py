...
def build_grid(resolution, device=None):
    """
    위치 인코딩을 위한 그리드 생성
    
    Args:
        resolution (tuple): 그리드 해상도 (H, W)
        device (torch.device): 텐서 장치
        
    Returns:
        torch.Tensor: 위치 인코딩 그리드 [1, H, W, 4]
    """
    if device is None:
...
class SoftPositionEmbed(nn.Module):
    ...
    def __init__(self, hidden_size, resolution, device=None):
        ...
        self.register_buffer('grid', build_grid(resolution, device))

    def forward(self, inputs):
        """
        Args:
            inputs (torch.Tensor): 입력 특징 [B, H, W, C]
            
        Returns:
            torch.Tensor: 위치 정보가 추가된 특징 [B, H, W, C]
        """
...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, num_slots=None):
        ...
        Args:
            ...
            num_slots (int, optional): 슬롯 수를 재정의
